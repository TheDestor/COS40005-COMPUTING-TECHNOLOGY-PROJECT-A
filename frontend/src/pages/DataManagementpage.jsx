...
const DataManagementPage = () => {
  ...
  const loadBackups = async () => {
    try {
      ...
      const list = await ky.get('/api/admin/backup/list', { headers }).json();
  ...
  const downloadBackup = async (filename) => {
    try {
      ...
      const blob = await ky.get(url, { headers }).blob();
  ...
  const confirmRunBackup = async () => {
    ...
    const backupPromise = (async () => {
      try {
        ...
        await ky.post('/api/admin/backup/run', { headers });
    ...
    toast.promise(backupPromise, {
      ...
      error: 'Backup failed. Please check your connection.',
    });

    // Use a final try/catch/finally to ensure the modal and loading state are reset
    try {
      await backupPromise;
    } catch (e) {
      // The error is already handled by the toast, so this catch block can be empty.
    } finally {
      setLoading(false);
  ...
  const loadAdminMetrics = async () => {
    try {
      ...
      const json = await ky.get('/api/admin/metrics/stats', { headers }).json();
      const d = json?.data || {};
  ...
  const loadBackupStatus = async () => {
    try {
      ...
      const s = await ky.get('/api/admin/backup/status', { headers }).json();
      setBackupStatus(s);
          ...
          <div className="confirm-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="confirm-action-title">
            <div className="confirm-modal">
              ...
              <div className="confirm-actions-dm">
                ...
                <button
                  className={confirmAction.type === 'download' ? 'modal-confirm-btn' : 'modal-delete-btn'}
                  onClick={async () => {
                  ...
                  disabled={loading}
                >
                  {loading ? 'Working…' : (confirmAction.type === 'download' ? 'Download' : 'Delete')}
                </button>
              </div>
            </div>
          </div>
